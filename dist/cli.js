...
async function validateInputs() {
    ...
    if (!ReporterFactory.isValidFormat(opts.output)) {
...
async function main() {
    try {
        ...
        logger.info(`Audit complete: ${report.services.length} services scanned, ` +
            `${report.summary.passedServices} passed, ${report.summary.failedServices} failed`);
        console.log(); // Empty line for spacing
        console.log(`✅ Audit complete: ${report.services.length} services scanned, ` +
            `${report.summary.passedServices} passed, ${report.summary.failedServices} failed`);
        // Generate reports
        const reporters = ReporterFactory.createReporters(opts.output);
        ...
        spinner.succeed(`Reports generated (${reporters.length} format${reporters.length > 1 ? 's' : ''})`);
